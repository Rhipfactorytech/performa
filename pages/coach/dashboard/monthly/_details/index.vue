        ...
        <template v-slot:default>
          ...
          <tbody>
            <tr v-for="(item, index) in messages" :key="index">
              ...
              <td>
               <!-- <nuxt-link :to="{ path: "coach-dashboard-monthly-details-' + item.monthlyId'"}> -->
                   <nuxt-link :to="{ path : '/coach/dashboard/monthly/details/detailsc', query: { monthid: item.monthlyId, partnerId: item.workid }}">
                  view
                </nuxt-link>
              </td>
              <td>
    ...
    async getmessages() {
      try {
        const res = await this.$axios.$get(
          `${this.$config.baseUrl}partner/monthlycheckin/coach/getall/${this.id}`
        )
        console.log(res)
