<template>
  <div>
    <v-tabs v-model="tab" background-color="#5465ff" grow dark icons-and-text>
      ...
      <v-tab href="#tab-2" class="text-capitalize" @click="getWeekly">
        Weekly report
      </v-tab>
      <v-tab href="#tab-3" class="text-capitalize" @click="getAppraisal"> Quaterly Appraisal </v-tab>
       <v-tab href="#tab-4" class="text-capitalize" @click="getmessages"> View feedback </v-tab>
    </v-tabs>
          ...
          <v-col cols="10" v-for="(item, index) in messages" :key="index">
            ...
            >
              <v-list-item>
                <v-list-item-content>
                  <v-list-item-subtitle
                    ><span class="font-weight-black">Date: </span>  {{item.date}}
                  </v-list-item-subtitle>
                  <v-list-item-subtitle
                    ><span class="font-weight-black">Subject: </span>{{ item.subject }}
                  </v-list-item-subtitle>
                    <v-list-item-subtitle>
...
export default {
  ...
  methods: {
     ...
     async getmessages() {
      try {
        const res = await this.$axios.$get(
         `${this.$config.baseUrl}partner/partner/get/messages`
    ...
    async getWeekly() {
      try {
        const res = await this.$axios.$get(
          `${this.$config.baseUrl}partner/weeklyreport/check/${this.workid}`
        )
        console.log(res.data)
