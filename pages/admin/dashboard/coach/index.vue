<template>
  <div>
    <v-tabs v-model="tab" background-color="#5465ff" grow dark icons-and-text>
      ...
      <v-tab href="#tab-1" class="text-capitalize"> List of coaches </v-tab>

      <v-tab href="#tab-2" class="text-capitalize"> Add Coach </v-tab>
    </v-tabs>
          ...
          <template v-slot:default>
            ...
            <tbody>
              <tr v-for="item in body" :key="item.workid" class="mt-2">
                ...
                <td>
                  <nuxt-link :to="'/admin/dashboard/coach/' + item.workId">
                    view profile
                  </nuxt-link>
                </td>
              </tr>
    ...
    async add_partner() {
      ...
      try {
        const res = await this.$axios.$post(
          `${this.$config.baseUrl}admin/godmode/coach/create`,
          {
            name: this.name,
            workid: this.workid,
